...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Launch a ClearML task for training a model.")
    parser.add_argument(
        "repo_url", type=str, help="URL of the repository containing the training script")
    parser.add_argument("branch_name", type=str,
                        help="Branch name of the repository")
    parser.add_argument("commit_hash", type=str,
                        help="Commit hash for the training script version")

    args = parser.parse_args()

    task = Task.init(
        project_name="ClearML Project", task_name="Model Training on Latest Script"
    )
